...
class EditProfileForm(forms.ModelForm):
	...
	first_name = forms.CharField(widget=forms.Textarea(attrs={'class': 'input is-small'}), max_length=50, required=False)
	last_name = forms.CharField(widget=forms.Textarea(attrs={'class': 'input is-small'}), max_length=50, required=False)
	location = forms.CharField(widget=forms.Textarea(attrs={'class': 'input is-small'}), max_length=25, required=False)
	url_linkedInn = forms.URLField(widget=forms.TextInput(attrs={'class': 'input is-small'}), max_length=60, required=False)
	url_github = forms.URLField(widget=forms.TextInput(attrs={'class': 'input is-small'}), max_length=60, required=False)
	profile_info = forms.CharField(widget=forms.Textarea(attrs={'class': 'input is-small'}), max_length=260, required=False)

	class Meta:
		model = Profile
		fields = ('picture', 'first_name', 'last_name', 'location', 'url_linkedInn', 'url_github', 'profile_info')